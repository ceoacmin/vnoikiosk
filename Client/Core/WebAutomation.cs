                ...
                (function() {
                    ...
                    function triggerLeaveContest() {
                        try {
                            ...
                            if (isDeepInContest) {
                                if (!document.getElementById('kiosk-warn-overlay')) {
                                    let warnOverlay = document.createElement('div');
                                    warnOverlay.id = 'kiosk-warn-overlay';
                                    warnOverlay.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.95); z-index:2147483647; display:flex; align-items:center; justify-content:center; flex-direction:column; backdrop-filter:blur(10px);';
                                    warnOverlay.innerHTML = `<div style=""background:#1E1B4B; padding:40px; border-radius:16px; border:2px solid #E11D48; max-width:500px; text-align:center; box-shadow:0 20px 50px rgba(0,0,0,0.5);""><h2 style=""color:#F43F5E; margin:0 0 15px 0; font-family:'Segoe UI', Arial, sans-serif; font-size:24px; font-weight:900;"">⚠️ CHƯA THỂ THOÁT</h2><p style=""color:#E2E8F0; font-family:'Segoe UI', Arial, sans-serif; font-size:15px; margin-bottom:25px; line-height:1.5;"">Hệ thống phát hiện bạn đang ở trong trang làm bài.<br>Vui lòng bấm nút <b>⮜ QUAY LẠI</b> góc trên bên trái để về <b>TRANG CHỦ</b> trước khi nộp bài.</p><button onclick=""document.getElementById('kiosk-warn-overlay').remove()"" style=""background:#334155; color:white; border:none; padding:12px 30px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:14px; font-family:'Segoe UI', Arial, sans-serif;"">ĐÃ HIỂU</button></div>`;
                        ...
                        try {
                            ...
                            if (formLeave) {
                                ...
                                if (realLeaveBtn && realLeaveBtn.tagName.toLowerCase() === 'a') {
                                     ...
                                     window.chrome.webview.postMessage(JSON.stringify({action: 'EXIT_CONTEST_SUCCESS'}));
                                }
                            }
                        } catch(e) { window.chrome.webview.postMessage(JSON.stringify({action: 'EXIT_CONTEST_SUCCESS'})); }
                    }
                    ...
                    function enforceUI() {
                        try {
                            ...
                            if (sessionStorage.getItem('kioskPendingLeave') === 'true') {
                                ...
                                if (!stillDeep) { performLeaveContest(); return; }
                            }
